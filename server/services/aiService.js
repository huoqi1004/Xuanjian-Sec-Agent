  ...
  try {
    ...
    switch (toolName) {
      case 'get_threat_intel': {
        const { iocType, value } = params;
        const threatIntelligence = require('../services/threatIntelligence');
        const result = await threatIntelligence.aggregateQuery(iocType, value);
        return { 
          success: true, 
          data: { 
      ...
      case 'query_shodan': {
        const { ip } = params;
        const threatIntelligence = require('../services/threatIntelligence');
        const result = await threatIntelligence.queryShodan(ip);
        if (result.data) {
          return { 
            success: true, 
            data: { 
        ...
        return { success: true, data: null, message: 'Shodan未收录该IP信息' };
      }
      case 'query_abuseipdb': {
        const { ip } = params;
        const threatIntelligence = require('../services/threatIntelligence');
        const result = await threatIntelligence.queryAbuseIPDB(ip);
        if (result.data) {
          return { 
            success: true, 
            data: { 
              ...
              country: result.data.country,
              isp: result.data.isp,
              recentReports: result.data.reports
            }, 
            message: `AbuseIPDB查询完成，滥用置信度: ${result.data.confidence}%` 
          };
        }
        return { success: true, data: null, message: 'AbuseIPDB未找到该IP报告' };
      }
      case 'query_virustotal': {
        const { iocType, value } = params;
        const threatIntelligence = require('../services/threatIntelligence');
        const result = await threatIntelligence.queryVirusTotal(iocType, value);
        if (result.data) {
          return { 
            success: true, 
            data: { 
              malicious: result.data.malicious, 
              suspicious: result.data.suspicious, 
              harmless: result.data.harmless,
              communityScore: result.data.communityScore,
              verdict: result.verdict
            }, 
            message: `VirusTotal查询完成，${result.data.malicious} 个引擎标记为恶意` 
          };
        }
        return { success: true, data: null, message: 'VirusTotal未找到该IOC信息' };
      }
      case 'get_scan_results': {
        const { task_id } = params;
        const results = db.all(`SELECT * FROM scan_results WHERE task_id = ?`, [task_id]);
        return { success: true, data: results, message: `找到 ${results.length} 条扫描结果` };
      }
      case 'get_alert_summary': {
        const { severity, limit = 10 } = params;
        ...
        const results = db.all(query, queryParams);
        return { success: true, data: results, message: `找到 ${results.length} 条告警` };
      }
      case 'get_baseline_results': {
        const { task_id } = params;
        const results = db.all(`SELECT * FROM baseline_results WHERE task_id = ?`, [task_id]);
        return { success: true, data: results, message: `找到 ${results.length} 条基线检查结果` };
      }
      case 'analyze_virus_hash': {
        const { hash } = params;
        const threatIntelligence = require('../services/threatIntelligence');
        const [vt, tb] = await Promise.allSettled([
          threatIntelligence.queryVirusTotal('hash', hash),
          threatIntelligence.queryThreatBook('hash', hash)
        ]);
        const sources = [];
        ...
        return {
          success: true,
          data: { hash, sources },
          message: `文件哈希 ${hash} 分析完成`
        };
      }
      case 'scan_file_with_ai': {
        const { fileHash, fileName, engineResults } = params;
        const summary = Object.values(engineResults || {}).map(e => 
          `${e.engine}: ${e.verdict} (置信度${(e.confidence * 100).toFixed(0)}%) - ${e.detail}`
        ).join('\n');
        return {
          success: true,
          data: { fileHash, fileName, summary },
          message: '文件AI分析完成'
        };
      }
      case 'generate_security_report': {
        const { type = 'daily' } = params;
        const report = await generateSecurityReport(getDashboardData(), type);
        return { success: true, data: report, message: '安全报告生成完成' };
      }
      case 'analyze_alerts': {
        ...
        return { success: true, data: result.content, message: '告警分析完成' };
      }
      case 'start_scan': {
        const { target_cidr, port_range, created_by } = params;
        if (!target_cidr || !port_range) {
        ...
        return { success: true, data: task, message: `扫描任务已创建: ${task.task_id}` };
      }
      case 'block_ip': {
        // 高危动作：仅由 Agent 人工确认后调用，落审计留痕
        ...
        return { success: true, data: { ip, duration, action: 'blocked' }, message: `已封禁 IP ${ip}` };
      }
      case 'account_lock': {
        // 高危动作：仅由 Agent 人工确认后调用，落审计留痕
        ...
        return { success: true, data: { username, action: 'locked' }, message: `已锁定账号 ${username}` };
      }
      default:
        return { success: false, error: `未知工具: ${toolName}` };
    }
  } catch (err) {
    logger.error(`工具执行失败 ${toolName}:`, err.message);
